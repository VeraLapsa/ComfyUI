...
// Outputs Node info when they are created.
    ...
    async registerNodes() {
        ...
        for (const nodeId in defs) {
            ...
            const node = Object.assign(
                function ComfyNode() {
                    // if debugging show useful info
                    // returning the same arg signature allows for building chains of functions
                    if (debugApp) {
                        this.onInputAdded = (input) => {
                            console.log(`${input.type} Input Added`, input);
                            return input;
                        };
                        this.onOutputAdded = (output) => {
                            console.log(`${output.type} Output Added`, output);
                            return output;
                        };
                    }

                    const inputs = nodeData["input"]["required"];
                    const opt_inputs = nodeData["input"]["optional"] || {};
                    for (const key in opt_inputs) {
                        opt_inputs[key][1] = {
                            ...(opt_inputs[key][1] || {}),
                            optional: true,
                        };
                    ...
                    for (const inputName in inputs) {
                        ...
                        const extra_info = inputData[1] || null;
                        if (extra_info?.optional) {
                            extra_info.color_off = extra_info.color_off ? extra_info.color_off : "#f4a261";
                            extra_info.color_on = extra_info.color_on ? extra_info.color_on : "#2a9d8f";
                            extra_info.shape = extra_info.shape ? extra_info.shape : "BOX_SHAPE";
                        }
                        if (extra_info?.shape) {
                            if (extra_info.shape in LiteGraph) {
                                extra_info.shape = LiteGraph[extra_info.shape];
                            }
                        }
                    ...
                    for (const output of nodeData["output"]) {
                        ...
                        if (typeof output === "object") {
                            const { type, label, extra_info } = output;
                            if (extra_info?.shape) {
                                if (extra_info.shape in LiteGraph) {
                                    extra_info.shape = LiteGraph[extra_info.shape];
                                }
                            ...
                            if (type) {
                                this.addOutput(label != "" ? label : type, type, extra_info);
                            }
                        }
                    }
                    ...
                    if (debugApp) {
                        console.log(`${nodeData.class_key} Node Added`, nodeData);
                    }
                },
                {
                    title: nodeData.title || nodeData.class_key, // don't break it for folks that don't mind using the NODE_CLASS_MAPPINGS key as the title
                    comfyClass: nodeData.class_key,
                }
            );
            node.prototype.comfyClass = nodeData.class_key;
            ...
            // category needs to stay out here for the add menu to be generated
