...
class RerouteNode {
    constructor() {
        ...
        this.title_mode = LiteGraph.NO_TITLE; // Added so horizontal works
...
app.registerExtension({
    name: "Comfy.TitledRerouteNode",
    registerCustomNodes() {
		class TitleRerouteNode extends RerouteNode {
			constructor() {
				super();
			}
			...
			computeSize() {
				return [
					this.properties.showOutputText && this.outputs && this.outputs.length
						? Math.max(90, LiteGraph.NODE_TEXT_SIZE * this.outputs[0].name.length * 0.6 + 40)
						: 90,
					26,
				];
			}
        ...
        // Load default visibility
        TitleRerouteNode.setDefaultTextVisibility(!!localStorage["Comfy.RerouteNode.DefaultVisibility"]);

        LiteGraph.registerNodeType(
            "TitledReroute",
            Object.assign(TitleRerouteNode, {
        ...
        TitleRerouteNode.category = "utils";
    },
});
